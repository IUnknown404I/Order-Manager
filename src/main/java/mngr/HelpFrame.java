...
=======
     ...
     * @param visualVar int option for needed case of information
...
>>>>>>> Stashed changes
    ...
    private void infoTextSetUp(int visualVar) {
        String defaultVar = "\n        Менеджер состоит из панели меню, панель заказов и панели инструментов (где находятся кнопки и информационное окно).\n\n"
...
<<<<<<< Updated upstream
                + "с директорией этого же заказа, где хранится все данные по нему (которые вы добавите, конечно).\n\n"
                + "        На панели инструментов имеется 4 кнопки, позволяющие добавить-редактировать-удалять данные по заказам (НО: редактирование заказов из архива запрещено! "
                + "Для полей архива возможен только просмотр\\изменение описание по двойному нажатию), а также обновлять базу (она же таблица, поскольку они взаимосвязаны). "
                + "Также, информационное окно, которое выводит сегодняшнюю дату для удобства и определённую сводку по актуальной базе.\n\n"
        ...
        String menuVar = "       В разделе \"Файл\" есть функция \"Восстановить системные файлы\", которая нужна для того, чтобы их из резерва скопировать и перенести "
                + "в директорию, с которой работает программа. Это нужно в тех случаях, если всплывает соответствующая ошибка об их отсутствии или при некорректном ручном "
=======
                + "с директорией этого же заказа, где хранятся все данные по нему (которые вы добавите, конечно). Кликнув правой кнопкой мыши по строке заказа, появится "
                + "меню с возможностью перенести данный заказ в архив.\n\n"
                ...
                + "        Меню состоит из категорий \"Файл\", где можно восстановить системные данные или же обновить (сопоставить и сделать актуальными) "
                + "системные файлы для корректной работы программы; \"Функции\", позволяет изменить (указать) путь к месту хранения всех данных, "
                + "обновить резерв данных и перенести все просроченные заказы из текущих проектов в архив; \"Справка\" - без комментариев.\n\n"
        ...
        String menuVar = "\n       В разделе \"Файл\" есть функция \"Восстановить системные файлы\", которая нужна для того, чтобы из резерва скопировать и перенести их "
...
<<<<<<< Updated upstream
                + "корректного функционирования программ, с одним условием: если по предыдущему пути не было каких-либо системных файлов, то они будут созданы пустыми. Это "
                + "означает, что при наличии по старому пути файлов, их контент будет перенесён и по новому, в противном- определённые поля заказов из новой директории "
...
>>>>>>> Stashed changes
                ...
                + "периодически проводите обновление дампа, чтобы всегда имелась запасная дирректория. Элемент \"перенести всю задержку в архив\" переносит все просроченные заказы "
                + "из текущих проектов в архив.\n\n"
...
=======
        String ordersWorkVar = "\n       В указанной корневой директории имеются такие папки, как: Текущие заказы, АРХИВ и config. В текущих заказах хранятся папки с данными по соответствующим "
>>>>>>> Stashed changes
                ...
                + "программно считаны: \"ГГГГ.ММ.ДД.{ПРОБЕЛ}{ЗАКАЗЧИК]\", где заказчик не должен быть пустым и обязателен пробел между датой и заказчиком. Дата в данном шаблоне - это "
                + "дата сдачи заказа.\n\n"
                ...
                + "очистка папки заказа, полностью, удаление её данных из системы и сброс соответствующей записи в таблице.";
        
        String errorsWorkVar = "\n       В процессе работы с программой следует руководствоваться справочной информацией, но даже так могут возникать определённые проблемы. "
                ...
                + "       При появлении ошибке о неналичии системных файлов следует либо указать новый корневой путь, либо восстановить их. Здесь нужно выбирать, исходя из ситуации: "
                + "если при выборе нового пути указать текущий, то данные тоже будут восстановлены, ну пустыми, а если у вас имеется дамп, то стоит выполнить восстановление. "
...
<<<<<<< Updated upstream
        String autofillVar = "       При заполнение полей заказов обязательно заполнять все поля, кроме описания. Но при всём этом, поле с датой сдачи автоматически заполняется текущей датой "
                + "с возможностью выбора последнего разряда года, поле срока сдачи - той же маской года, а оставшиеся цифры нулями, а если оставить поле Заказчика пустым, то "
=======
        String autofillVar = "\n       При заполнении полей заказов обязательно заполнять все поля, кроме описания. Но при всём этом, поле с датой сдачи автоматически заполняется текущей датой "
...
>>>>>>> Stashed changes
                ...
                + "       Следует напомнить, что следует соблюдать обязательные форматы имени для всех данных, с которыми работает менеджер: директория для действующих заказов - "
                + "\"Текущие заказы\", для архива - \"АРХИВ\", для системных файлов и дампа - \"config\", для дампа - \"dump\"; для папой заказов - \"ГГГГ.ММ.ДД.{ПРОБЕЛ}{ЗАКАЗЧИК]\", "
